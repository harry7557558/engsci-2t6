...
def game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate, score_to_win, display):
    ...
    while max(score) < score_to_win:
        ...
        if frame_id % max(int(clock_rate/120+0.5), 1) != 0:
            continue
...
def init_game():
    ...
    import sys
    sys.path.insert(1, '/other_peoples_code/loluwot_pongai')
    import other_peoples_code.lindyzh_pongbot
    import other_peoples_code.loluwot_pongai.better_ai
